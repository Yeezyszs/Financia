...
export function History({
  ...
  const [duplicateFile, setDuplicateFile] = useState<{ name: string; content: string } | null>(null);
  ...
  const upload = useCallback(
    async (name: string, content: string, force = false) => {
      ...
      try {
        const outcome = await api.createImport({ accountId, filename: name, content, force });
        setResult(outcome);
      ...
      <div className="card stack" style={{ marginBottom: 20 }}>
        ...
        {result ? (
          <div className="notice">
            ...
            <b>{result.rowsDuplicated}</b> já existiam · <b>{result.categorized}</b> categorizadas
            automaticamente
            {result.periodStart ? (
              <>
                {' '}
                · período {date(result.periodStart)} a {date(result.periodEnd ?? result.periodStart)}
              </>
            ) : null}
          </div>
        ) : null}
                ...
                <tr key={record.id}>
                  ...
                  <td>
                    {record.status === 'failed' ? (
                      <span className="tag" title={record.errorMessage ?? ''} style={{ color: 'var(--danger)' }}>
                        falhou
                      </span>
                    ) : (
                      <span className="tag">{record.status === 'completed' ? 'ok' : record.status}</span>
                    )}
                  </td>
                </tr>
              ))
